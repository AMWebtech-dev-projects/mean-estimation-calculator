...
export class JwtService {
  ...
  /** This method will save config data to local storage*/
  saveConfig(configDetails: any) {
    window.localStorage.setItem(
      environment.calculationConfig,
      CryptoJS.AES.encrypt(configDetails, this.jwtTokenKey).toString()
    );
  }

  /** This method will get config data information from local storage*/
  getConfig(): any {
    this.jwtTokenKey = this.getKeyLS(environment.jwtTokenKey);
    this.calculationConfig = this.getKeyLS(environment.calculationConfig);
    if (this.jwtTokenKey) {
      this.calculationConfigInfo = JSON.parse(
        CryptoJS.AES.decrypt(this.calculationConfig, this.jwtTokenKey).toString(
          CryptoJS.enc.Utf8
        )
      );
      return this.calculationConfigInfo;
    } else {
      return false;
    }
  }
  ...
  destroyToken() {
    ...
    window.localStorage.removeItem(environment.calculationConfig);
  }
